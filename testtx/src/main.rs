...
use elements_miniscript::bitcoin::secp256k1::{Secp256k1, SecretKey};
...
fn main() -> Result<(), String> {
    ...
    let tlbtc_assetid = elements::AssetId::from_slice(&[
        0x14, 0x4c, 0x65, 0x43, 0x44, 0xaa, 0x71, 0x6d,
        0x6f, 0x3a, 0xbc, 0xc1, 0xca, 0x90, 0xe5, 0x64,
        0x1e, 0x4e, 0x2a, 0x7f, 0x63, 0x3b, 0xc0, 0x9f,
        0xe3, 0xba, 0xf6, 0x45, 0x85, 0x81, 0x9a, 0x49, 
    ]).unwrap();
    // from node
    let genesis_hash = elements::BlockHash::from_byte_array([
        0xa7, 0x71, 0xda, 0x8e, 0x52, 0xee, 0x6a, 0xd5,
        0x81, 0xed, 0x1e, 0x9a, 0x99, 0x82, 0x5e, 0x5b,
        0x3b, 0x79, 0x92, 0x22, 0x55, 0x34, 0xea, 0xa2,
        0xae, 0x23, 0x24, 0x4f, 0xe2, 0x6a, 0xb1, 0xc1, 
    ]);
    ...
    let (pk, _parity) = sk.x_only_public_key(&Secp256k1::new());
    ...
    let sighash = {
        ...
        cache.taproot_script_spend_signature_hash(
            0,
            &elements::sighash::Prevouts::All(&funding_tx.output),
            elements::sighash::ScriptPath::new(
                script,
                0, // codesep pos
                0xbe, // from elements source
            ),
    ...
    let satisfaction = descriptor.satisfy(&mut tx.input[0], &sk);
